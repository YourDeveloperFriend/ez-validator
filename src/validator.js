...
import UserError, {ValidationError} from 'ez-error'
...
export default class Validator {
  ...
  async validateObject(validationInfo, data, context) {
    var errors = {};
  ...
  async validateField(errors, info, key, data, context) {
    ...
    if(info.required || !_.isUndefined(data[name])) {
      ...
      if(isObject || isArray) {
        if(_.isObject(data[name]) == isObject && _.isArray(data[name]) == isArray) {
          ...
          var errors = await this.validateObject(info.type, data[name], context);
          if(errors) {
        ...
        await this.runAValidation(errors, typeValidator, context, data[name], null, {
          key: key,
          validationData: info
        });
      }
  ...
  async runAValidation(errors, fnName, context, value, validationInfo, info) {
    if(this.validations[fnName]) {
      try {
        await this.validations[fnName].call(context, value, validationInfo, info);
      } catch(e) {
